...
export default function Topbar({ user }: { user: any }) {
  ...
  return (
    <>
      <header className="h-14 border-b border-zinc-800/80 bg-zinc-950 px-4 md:px-8 flex items-center justify-between sticky top-0 z-40 select-none">
        <div className="flex items-center gap-3">
          {/* Mobile Drawer Hamburger Menu Button */}
          <button
            onClick={() => setMobileNavOpen(true)}
            aria-label="Open Navigation Menu"
            className="md:hidden p-2 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-900 rounded-lg transition cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          {/* Credits Pill */}
          <div className={"px-3 py-1 rounded-full text-[11px] sm:text-xs font-semibold border shadow-sm transition-all duration-150 whitespace-nowrap flex-shrink-0 " + (
            user.credits < 100 
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="w-8 h-8 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center justify-center cursor-pointer transition select-none outline-none border border-indigo-500/30 shadow-md"
            >
